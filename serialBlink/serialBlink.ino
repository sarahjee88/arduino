void setup() {
  ...
  Serial.begin(9600);  // start the seiial and set the baud rate to 9600
  Serial.setTimeout(10);  // set arduino to scan the buffer per 0.001 sec
...
void loop() {
  ...
  if (Serial.available() > 0) { // check anything for serial buffer
    String command; // creates string variable called command
    command = Serial.readString(); // set arduino to read message that are sent
